...
import {
  ...
  buildEnrichmentRowFromAnalysis,
  upsertReportRow,
  HAIKU_MODEL,
} from "./auto-report";
import { normalizeAddress } from "./extract-address";

// Dashboard path uses Haiku 4.5 for speed + cost + higher Tier 1
// rate-limit ceiling. Sonnet stays the default for the homepage PDF
// flow (pages/api/submit.js), where output quality is the deliverable.
const DASHBOARD_MODEL = HAIKU_MODEL;
...
export async function runAnalysisForHome({
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    try {
      ...
      const analysis = await analyzeWithClaude(listing_url, { model: DASHBOARD_MODEL });
      ...
      // Phase 2: enrichment (lifestyle / schools / commute / taxes).
      // Fire-and-forget within the same waitUntil. Failure is non-fatal —
      // the user already has the core report on screen.
      runEnrichmentForReport({
        supabase,
        address: finalAddress,
        listing_url,
      }).catch((err) =>
        console.warn("[runAnalysisForHome] enrichment failed (non-fatal):", err)
      );
...
// Phase 2: enrichment pass. Fires after the core report is on screen.
// Single attempt, best-effort. If it succeeds, merges lifestyle / school
// / commute / tax fields into the existing reports row. If it fails or
// the function gets killed mid-run, the user keeps the Phase 1 report.
export async function runEnrichmentForReport({ supabase, address, listing_url }) {
  console.log("[runEnrichmentForReport] start", {
    address,
    started_at: new Date().toISOString(),
  });
  try {
    const analysis = await analyzeEnrichment(listing_url, address, { model: DASHBOARD_MODEL });
    const fields = buildEnrichmentRowFromAnalysis(analysis || {});
    if (Object.keys(fields).length === 0) {
      console.log("[runEnrichmentForReport] no fields returned, skipping update");
      return { status: "empty" };
    }
    const { error } = await supabase
      ...
      .eq("address", address);
    if (error) {
      console.error("[runEnrichmentForReport] update error:", error);
      return { status: "failed", error: error.message };
    }
    console.log("[runEnrichmentForReport] success", {
      address,
    ...
    console.error("[runEnrichmentForReport] error:", err);
