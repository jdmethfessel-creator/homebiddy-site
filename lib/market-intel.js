...
// Strip parenthetical descriptors and known suffixes from a neighborhood
// name so display matches the canonical short form Claude is now prompted
// for. Cleans legacy rows that still have e.g. "SoSo (Belair Historic
// District)" → "SoSo" without needing a data migration.
export function cleanNeighborhood(name) {
  if (!name) return name;
  let s = String(name).trim();
  // Drop anything in parentheses: "SoSo (Belair Historic District)" → "SoSo"
  s = s.replace(/\s*\([^)]*\)\s*/g, " ").trim();
  // Drop trailing descriptor words: "Old Northwood Historic District" → "Old Northwood"
  s = s.replace(/\s+(historic district|district|neighborhood|area|sector)\s*$/i, "").trim();
  // Collapse double spaces left from prior replacements
  s = s.replace(/\s{2,}/g, " ");
  return s;
}
...
export function getMarketKey(report) {
  ...
  if (report.neighborhood && String(report.neighborhood).trim()) {
    return cleanNeighborhood(report.neighborhood);
  }
...
export function aggregateNeighborhood(reports) {
  ...
  // Avg lot $/sqft for the submarket — same fallback approach.
  const lotPsfs = valid
    .map((r) =>
      Number(r.price_per_lot_sqft) ||
      (r.lot_size_sqft ? r.asking_price / r.lot_size_sqft : null)
    )
    .filter((v) => v != null && !isNaN(v));
  const avgLotPsf =
    lotPsfs.length > 0
      ? Math.round(lotPsfs.reduce((s, v) => s + v, 0) / lotPsfs.length)
      : null;
