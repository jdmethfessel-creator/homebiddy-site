...
CRITICAL: Return ONLY the raw JSON object. No preamble text. No markdown fences. No ** bold markers. No mathematical expressions as values -- compute all math and emit only the resulting number.

CRITICAL: Always return a valid JSON object. Never return plain English, never apologize, never ask clarifying questions. If data is missing, omit that field. If the listing cannot be accessed, return: {"error": "listing_not_found", "message": "one sentence reason"}

Never include mathematical expressions like '5 + (x - 1) * 20' as JSON values -- always compute the result and emit only the number.`;
...
function extractFinalJSON(response) {
  ...
  // Also strip ** bold markers since Claude occasionally wraps numeric
  // values like **9.2** which JSON.parse rejects.
  const fenced = full.match(/```(?:json)?\s*([\s\S]*?)```/);
  if (fenced) {
    const cleanFenced = fenced[1].trim().replace(/\*\*/g, '');
    const fencedTry = tryParse(cleanFenced);
