...
- offer_range_flagged: set this to TRUE if your calculated offer_low is more than 25% below asking_price. Extreme discounts on fresh listings are almost always a sign of bad source data (wrong asking, missing context, stale comps). When true, include offer_range_flag_note explaining what looks off (e.g. "Asking $1.2M, but comps trend $700-800K — verify the listing is current and not a typo").
- If the discount is within normal range (≤ 25%), omit both fields.
...
ROUND to one decimal AND CLAMP the final value to the [1, 10] range — never emit 0 or any value below 1. Even a 'no-leverage' listing where every sub-score is 0 should report a final negotiability_score of 1 (the floor), NOT a raw sub-component value.
...
// hallucinated 12 doesn't blow up the final number, and the result is
...
function recomputeNegotiabilityScore(breakdown) {
  ...
  const cuts = numOrNull(breakdown.price_cut_score);
  const zest = numOrNull(breakdown.zestimate_gap_score);
  const psf = numOrNull(breakdown.price_per_sqft_score);
  if (dom == null || cuts == null || zest == null || psf == null) return null;
  const weighted =
    0.30 * subClamp(dom) +
    0.25 * subClamp(cuts) +
    0.25 * subClamp(zest) +
...
export function buildReportRowFromAnalysis(a, address) {
  const data = {
    ...
    // Data-quality flag: extreme discounts (>25%) get flagged with a note.
    offer_range_flagged:
      a.offer_range_flagged === true ||
      shouldFlagOfferRange(a.asking_price, a.offer_low),
    offer_range_flag_note: a.offer_range_flag_note || null,
  };
  return {
    address,
