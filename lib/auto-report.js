...
LISTING PRICE
- listing price and price history: read the full Price History table on the listing page. The CURRENT asking price is the most recent 'Price change' or active listing price at the top of the table -- never the original 'Listed for sale' price. Count only 'Price change' events as price_cuts. Build cut_history as an array of strings from those rows, most recent first, format: '$2,595,000 → $2,395,000 (Feb 2024)'. Also note the most recent 'Sold' row for last_sold_price and last_sold_year.
...
SELLER STORY
- seller_story (string): 2-3 sentence plain-English narrative that reconstructs what happened with this listing. Use the price history and tax history to tell the story. Include: when they bought and for how much, whether they likely renovated (infer from buy price vs list price gap and listing language), how long it has been listed, how many cuts and total $ dropped, and what the likely seller cost-basis floor is.

  Example: 'Seller bought in 2019 for $659K, likely invested $400-600K in renovation based on listing language and the gap to their $2.595M original ask. After 2 years and 4 cuts totaling $700K, they are now at $1.895M -- still well above their estimated $1.1-1.2M cost basis. Seller has room to negotiate but is unlikely to go below $1.65-1.70M without a compelling offer.'

  Keep it factual and grounded in the data. Do not speculate beyond what the numbers support.
...
const HOMEPAGE_SYSTEM_PROMPT = `You are HomeBiddy, a real estate offer analyst. Given a Zillow or Realtor.com listing URL, use web search to research the listing and return a JSON object with the fields listed below.
...
LISTING PRICE
- listing price and price history: read the full Price History table on the listing page. The CURRENT asking price is the most recent 'Price change' or active listing price at the top of the table -- never the original 'Listed for sale' price. Count only 'Price change' events as price_cuts. Build cut_history as an array of strings from those rows, most recent first, format: '$2,595,000 → $2,395,000 (Feb 2024)'.
    ...
    tools: [
      {
        ...
        name: "web_search",
        // Tight cap for the homepage flow (HOMEPAGE_SYSTEM_PROMPT, 3
        // searches). Dashboard callers that pass the full SYSTEM_PROMPT
        // get the same cap; lifestyle / schools / taxes are pulled by
        // analyzeEnrichment() in Phase 2.
        max_uses: 3,
      },
    ],
    messages: [
...
// =====================================================================
// PHASE 2 — Enrichment pass. Looks up lifestyle, schools, commute, and
// historical tax data after the core report is already on screen. Runs
// best-effort: if it fails the user still has the Phase 1 report.
// =====================================================================

const ENRICHMENT_SYSTEM_PROMPT = `You are HomeBiddy's enrichment service. The core report for a property already exists. Use web search to gather ONLY the supplementary fields below and return a single JSON object with the ones you find. Omit any field you cannot confidently find; do NOT invent values.

Fields to extract:

TAXES + SALE HISTORY
- last_sold_price (integer), last_sold_year (integer)
- tax_assessed_value (integer) — current county assessed value
- annual_taxes_current (integer) — current annual property tax bill
- flood_zone (string) — FEMA designation like "X", "AE", "VE"

Rules:
- All money values as integers (USD).
- Cap search usage tightly. If a field isn't surfacing in the first 1-2 searches, omit it.
- Return ONLY valid JSON, no markdown, no commentary.`;

export async function analyzeEnrichment(listing_url, address, { model } = {}) {
  if (!process.env.ANTHROPIC_API_KEY) {
    throw new Error("ANTHROPIC_API_KEY is not set");
  }
  const client = new Anthropic({ apiKey: process.env.ANTHROPIC_API_KEY });
    ...
    messages: [
      {
        role: "user",
        content:
          `Property address: ${address}\nListing URL: ${listing_url || "(none)"}` +
          `\n\nFetch the enrichment fields and return the JSON object.`,
      },
    ],
  });
...
export function buildReportRowFromAnalysis(a, address) {
  const data = {
    ...
    seller_story: a.seller_story || null,
  ...
  return {
    ...
    walk_score: a.walk_score ?? null,
    transit_score: a.transit_score ?? null,
    bike_score: a.bike_score ?? null,
    elementary_school: a.elementary_school ?? null,
    elementary_rating: a.elementary_rating ?? null,
    middle_school: a.middle_school ?? null,
    middle_rating: a.middle_rating ?? null,
    high_school: a.high_school ?? null,
    high_rating: a.high_rating ?? null,
    school_rating_avg: a.school_rating_avg ?? null,
    commute_to_downtown_min: a.commute_to_downtown_min ?? null,
    nearest_grocery_min: a.nearest_grocery_min ?? null,
    data,
  };
}
