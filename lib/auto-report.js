...
- seller_story (string): 2-3 sentence plain-English narrative that reconstructs what happened with this listing. Use the price history and tax history to tell the story. Include: when they bought and for how much, whether they likely renovated (infer from buy price vs list price gap and listing language), how long it has been listed, how many cuts and total $ dropped, and what the likely seller cost-basis floor is.
  ...
  Keep it factual and grounded in the data. Do not speculate beyond what the numbers support. Derive seller_story entirely from data already found -- do not perform an additional web search for it.
...
export async function analyzeWithClaude(listing_url, { model, systemPrompt } = {}) {
  ...
  const response = await client.messages.create({
    ...
    // expanded field set + seller_story; homepage stays tight at 4096.
    ...
    tools: [
      {
        ...
        name: "web_search",
        // Homepage stays tight at 3. Dashboard (systemPrompt) gets one
        // extra search to cover the price-history table + tax-history
        // lookups without starving the comp queries.
        max_uses: systemPrompt ? 4 : 3,
      },
    ],
    messages: [
      {
