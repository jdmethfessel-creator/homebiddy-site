...
LISTING PRICE
- Listing price: use the most recent price from the price history table -- this is the top row, which will be a 'Price change' event if cuts have occurred. Never use the original 'Listed for sale' price as the current asking price. Count the number of 'Price change' rows as price_cuts. Build cut_history from those rows in order, most recent first.
...
LIFESTYLE
- walk_score (integer 0-100) — search "[address] walk score" (walkscore.com)
- transit_score (integer 0-100) — same source
- bike_score (integer 0-100) — same source

SCHOOLS
- elementary_school (string), elementary_rating (number out of 10)
- middle_school (string), middle_rating
- high_school (string), high_rating
- school_rating_avg (number, one decimal) — mean of the three ratings
Source: greatschools.org for the property's address; fall back to niche.com.

COMMUTE
- commute_to_downtown_min (integer) — peak-hour drive minutes to the city's main CBD
- nearest_grocery_min (integer) — drive minutes to the nearest full-service grocery
