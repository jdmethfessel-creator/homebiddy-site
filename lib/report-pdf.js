...
const styles = StyleSheet.create({
  ...
  // Slightly tighter padding + font so all 4 fit on page 1 below the
  // comps table without spilling — page 2 is reserved for the script
  ...
  insightBody: { flex: 1, fontSize: 9, lineHeight: 1.4, color: C.navy },
    ...
    >
      <Page size="LETTER" style={styles.page}>
        ...
        {/* Page 2 — single forced break with 40pt of blank space at the
            top. Page 1 carries header + offer + stats + comps + all 4
            insights; page 2 holds the script + questions. The fixed
            footer block below renders on every page. */}
        {(data.negotiation_script || questions.length > 0) && (
          <View break style={{ paddingTop: 40 }}>
            {data.negotiation_script && (
              <>
                <Text style={[styles.sectionH, { marginBottom: 14 }]}>
                  Negotiation Script
                </Text>
