...
const styles = StyleSheet.create({
  ...
  // Items 3 and 4 land at the top of page 2 — give them a bit more
  // breathing room (vertical) without touching the page-1 cadence.
  insightExtraPad: {
    paddingVertical: 14,
  ...
  return (
    ...
    >
      <Page size="LETTER" style={styles.page}>
          ...
          <>
            ...
            {insights.map((line, i) => (
              <View
                key={i}
                break={i === 2}
                style={i === 2 ? { paddingTop: 40 } : undefined}
              >
                <View
                  style={[styles.insight, i >= 2 && styles.insightExtraPad]}
                  wrap={false}
                >
        ...
        {/* Negotiation script — left-bordered quote block. Header carries
            inline top + bottom margin so the page-2 cadence has more
            breathing room than page-1 sections need. */}
        {data.negotiation_script && (
          <>
            <Text style={[styles.sectionH, { marginTop: 26, marginBottom: 14 }]}>
              Negotiation Script
            </Text>
